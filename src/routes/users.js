...
router.get('/search', optionalAuth, async (req, res) => {
  ...
  try {
    const users = await dbAll(
      `SELECT id,name,handle,avatar_url,bio FROM users
       WHERE name ILIKE $1 OR handle ILIKE $1 LIMIT 20`,
      [`%${q}%`]
    );
    res.json({ users });
...
router.get('/:id', optionalAuth, async (req, res) => {
  try {
    const user = await dbGet(
      'SELECT id,name,handle,avatar_url,bio,created_at FROM users WHERE id=$1',
      [req.params.id]
    );
    ...
    const [followers, following, videosCount] = await Promise.all([
      ...
      dbGet('SELECT COUNT(*) as c FROM videos WHERE user_id=$1 AND status=$2', [user.id, 'published']),
    ]);
...
router.get('/:id/followers', optionalAuth, async (req, res) => {
  try {
    const users = await dbAll(
      `SELECT u.id,u.name,u.handle,u.avatar_url FROM follows f
